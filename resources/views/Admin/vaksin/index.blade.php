...
<div class="row">
								    	...
								    	<div class="modal-content">
								      		...
								      		<form method="POST" class="w-100" id="editForm" enctype="multipart/form-data" action="{{url('admin/data-vaksin/edit')}}">
									        	<div class="modal-body">
													...
													<div>
														<img id="imageUpload2" class="img-thumbnail shadow rounded mx-auto mb-3 d-block d-none skeleton">
													</div>
												</div>
	...
	<div class="col-md-3 mb-2 card">
		<form id="createForm" method="POST" class="mt-2 mb-5 w-100" enctype="multipart/form-data">
			...
			<div>
				<img id="imageUpload" class="img-thumbnail shadow rounded mx-auto mb-3 d-block d-none skeleton">
			</div>
		...
		$("#createForm").submit(function(e){
  			...
  			$.ajax({
				...
				success:function(response){
					if (response) {
						$("#table_id tbody").append('<tr id="vid'+response.id_vaksin+'"><td id="nama-vaksin">'+response.nama_vaksin+'</td><td id="img-vaksin">@if('+response.img+')<img id="imageContent'+response.id_vaksin+'" width="100" class="img-thumbnail rounded mx-auto d-block skeleton">@endif</td><td id="deskripsi-vaksin">'+response.deskripsi+'</td><td id="harga-vaksin" class="fw-bold">Rp '+response.harga+'</td><td id="kontrol-vaksin"><button type="button" class="btn btn-warning text-dark w-100 mb-1" onclick="editVaksin('+response.id_vaksin+')" data-bs-toggle="modal" data-bs-target="#editModal">Edit</button><form id="deleteForm" method="POST" action="data-vaksin/delete/'+response.id_vaksin+'">@csrf @method("post")<button type="submit" class="btn btn-danger text-white  w-100" name="submit">Delete</button></form></td></tr>');
